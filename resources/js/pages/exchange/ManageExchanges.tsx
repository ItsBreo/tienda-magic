      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div>
            <div className="mb-4 flex items-center justify-between border-b border-border/50 pb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{received.length} RESULTADOS</span>
            </div>
            ...
            <div className="space-y-4">
              ...
              {received.map((req: any) => (
                <div key={req.id} className="border border-border rounded-xl p-4 bg-card w-full flex flex-col group transition-all duration-300 hover:shadow-2xl hover:border-primary/30 hover:-translate-y-1">
                  ...
                  <div className="flex items-center gap-3 border-b border-border/50 pb-3 mb-3">
                    ...
                    <p className="text-xs font-bold text-primary flex-1 truncate">{req.user?.name} ofrece:</p>
                  </div>
                    ...
                    <div className="w-8 h-12 bg-background rounded overflow-hidden shrink-0 grayscale opacity-80">
                      {req.exchange?.offered_card?.card?.image_url && (
                        <img 
                          ...
                          alt="tu carta" className="w-full h-full object-cover" 
                        />
                      )}
                    </div>
                    <div className="min-w-0">
          ...
          <div>
            <div className="mb-4 flex items-center justify-between border-b border-border/50 pb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{sent.length} RESULTADOS</span>
            </div>
                      ...
                      <div className="flex justify-between items-center bg-accent/40 p-2.5 rounded-lg border border-border/50">
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          {req.offered_card?.card?.image_url && (
                            <img 
                              src={req.offered_card.card.image_url.startsWith('http') ? req.offered_card.card.image_url : `/storage/${req.offered_card.card.image_url}`} 
                              className="w-5 h-7 object-cover rounded" alt="carta"
                            />
                          )}
        ...
        <div className="mt-16">
          <div className="mb-6 border-b border-border pb-2 flex items-center justify-between">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{completed.length} RESULTADOS</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {completed.map((hist: any) => {
              ...
              return (
                <div key={`hist-${hist.id}`} className="border border-border rounded-xl p-4 bg-card w-full flex flex-col group transition-all duration-300 hover:shadow-2xl hover:border-primary/30">
                  <div className="flex justify-between items-center mb-3">
