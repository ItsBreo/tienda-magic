...
import SearchableSelectAsync from '@/components/common/SearchableSelectAsync';
...
export default function Exchanges() {
  ...
  // Fetch paginado para el buscador async de "¿Qué quieres a cambio?"
  const fetchCards = useCallback(async (search: string, page: number) => {
    console.log('[Exchanges] Buscando cartas:', { search, page });
    try {
      const data = await apiService.getAllCards(page, search);
      console.log('[Exchanges] Respuesta API:', data);
      const items = Array.isArray(data) ? data : (data?.data || []);
      const lastPage = data?.last_page || data?.meta?.last_page || 1;
      return {
        data: items.map((c: any) => ({ value: c.id, label: c.name })),
        lastPage,
      };
    } catch {
  ...
  // Opciones del inventario para el SearchableSelect estático
  const inventoryOptions = (inventory || [])
    .filter((inv: any) => (inv.quantity - inv.quantity_locked) > 0)
    .map((inv: any) => ({
      value: inv.id,
      label: inv.card?.name || 'Carta desconocida',
      sublabel: `${inv.quantity - inv.quantity_locked} disponibles`,
    }));

  // Opciones del inventario para el modal de "Proponer Intercambio"
  const inventoryOptionsFiltered = (inventory || [])
    .filter((inv: any) => {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-border pb-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-foreground text-background font-black text-xs uppercase tracking-widest h-11 px-8 rounded-xl hover:bg-primary hover:text-primary-foreground hover:scale-105 transition-all shadow-md"
            >
              Publicar Oferta
            </button>
            ...
            <a
              ...
              className="bg-accent text-foreground font-black text-xs uppercase tracking-widest h-11 px-8 py-3.5 rounded-xl hover:bg-background hover:border-primary/50 transition-all border border-border shadow-sm text-center leading-none"
            >
              Gestión
            </a>
          </div>
            ...
            >
              ...
              <div className="group/img flex items-center justify-center bg-accent/5 rounded-lg p-2 h-48 relative overflow-hidden">
                {exchange.offered_card?.card?.image_url ? (
                  <img
                    src={exchange.offered_card.card.image_url.startsWith('http')
                      ? exchange.offered_card.card.image_url
                      : `/storage/${exchange.offered_card.card.image_url}`}
              ...
              <div className="mt-4 flex flex-col flex-1">
                <div className="flex justify-between items-start gap-2 mb-1">
                  <h3 className="text-foreground font-forum font-bold text-lg leading-tight line-clamp-2 h-[3rem] overflow-hidden flex-1 tracking-tight">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  />
                  <p className="text-[11px] font-black tracking-wider uppercase text-muted-foreground truncate flex-1">
                    {exchange.user?.name}
                  </p>
                ...
                <div className="w-full mt-3">
                  {exchange.user_id === user?.id ? (
                    <button
                      disabled
                      className="h-9 w-full bg-accent text-muted-foreground font-black text-[10px] uppercase tracking-widest border border-border cursor-not-allowed rounded-lg opacity-50"
                    >
                      Tus artículos
                    </button>
                  ) : (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative z-[70] bg-card/60 backdrop-blur-md border border-border p-8 rounded-3xl w-full max-w-md shadow-2xl"
            >
              ...
              <p className="text-muted-foreground text-sm mb-6">
                Ofrece una carta a <span className="text-primary font-bold">{selectedExchange?.user?.name}</span>.
              </p>
              ...
              <div className="mb-6 space-y-2">
                ...
                <SearchableSelect
                  ...
                  onChange={(val) => setSelectedInventoryCardId(val)}
                  placeholder={loadingInventory ? 'Cargando inventario...' : '-- Elige una carta de tu colección --'}
                  emptyText="No hay cartas disponibles"
                  disabled={loadingInventory}
                  options={inventoryOptionsFiltered}
                />
                {selectedExchange?.requested_card_id &&
                  inventoryOptionsFiltered.length === 0 &&
                  !loadingInventory && (
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={
                    !selectedInventoryCardId ||
                    (selectedExchange?.requested_card_id && inventoryOptionsFiltered.length === 0)
                  }
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
              className="relative z-[70] bg-card/60 backdrop-blur-md border border-border p-8 rounded-3xl w-full max-w-[500px] shadow-2xl"
            >
              ...
              <div className="space-y-4 mb-6">
                <div className="space-y-2">
                  ...
                  <SearchableSelect
                    ...
                    onChange={(val) => setNewOfferCardId(val)}
                    placeholder="-- Elige qué darás (Requerido) --"
                    emptyText="No hay cartas disponibles"
                ...
                <div className="space-y-2">
                  ...
                  <SearchableSelectAsync
                    ...
                    onChange={(val) => setNewRequestCardId(val)}
                    fetchOptions={fetchCards}
                    placeholder="Cualquier carta (Abierto a ofertas)"
                    emptyText="No se encontraron cartas"
                  />
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-black opacity-60">
                    O déjalo en 'Cualquier carta' para ofertas libres.
                  </p>
                </div>
              </div>
